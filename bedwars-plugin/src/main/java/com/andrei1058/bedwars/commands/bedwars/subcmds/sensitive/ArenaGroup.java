...
public class ArenaGroup extends SubCommand {
    ...
    public ArenaGroup(ParentCommand parent, String name) {
        ...
        setDisplayInfo(Misc.msgHoverClick("§8- §e/" + getParent().getName() + " " + getSubCommandName()+" §8- §bClick for details", "§eManage arena groups.",
    ...
    public boolean execute(String[] args, CommandSender s) {
        ...
        if (args.length < 2 && (args.length < 1 || !args[0].equalsIgnoreCase("list"))) {
            ...
            p.sendMessage("§aGroup has been created!");
            ...
            if (!groups.contains(args[1])) {
                p.sendMessage("§cThis group doesn't exist!");
                return true;
            }
            groups.remove(args[1]);
            config.set(ConfigPath.GENERAL_CONFIGURATION_ARENA_GROUPS, groups);
            p.sendMessage("§cGroup has been deleted!");
            ...
            p.sendMessage("§aAvailable arena groups:");
            p.sendMessage("§8- §7Default");
            for (String gs : groups) {
                p.sendMessage("§8- §7" + gs);
            }
            ...
            if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_ARENA_GROUPS) != null) {
                if (config.getYml().getStringList(ConfigPath.GENERAL_CONFIGURATION_ARENA_GROUPS).contains(args[2])) {
                    ...
                    if (!arena.exists()) {
                        p.sendMessage("§cArena " + args[1] + " doesn't exist!");
                        return true;
                    }
                    ...
                    p.sendMessage("§aArena " + args[1] + " has been added to the group: " + args[2]);
                } else {
                    p.sendMessage("§6 ▪ §7There isn't any group called: " + args[2]);
                    Bukkit.dispatchCommand(p, "/bw list");
                }
            } else {
                p.sendMessage("§cThere isn't any group called: " + args[2]);
                Bukkit.dispatchCommand(p, "/bw list");
            }
        } else {
            sendArenaGroupCmdList(p);
    ...
    private void sendArenaGroupCmdList(Player p) {
        p.spigot().sendMessage(Misc.msgHoverClick("§8- §e/" + getParent().getName() + " " + getSubCommandName() + " create <groupName>",
                "§eCreate an arena group. More details on our wiki.", "/" + getParent().getName() + " " + getSubCommandName() + " create",
                ClickEvent.Action.SUGGEST_COMMAND));
        p.spigot().sendMessage(Misc.msgHoverClick("§6 ▪ §7/" + getParent().getName() + " " + getSubCommandName() + " list",
                "§eView available groups.", "/" + getParent().getName() + " " + getSubCommandName() + " list",
                ClickEvent.Action.RUN_COMMAND));
        p.spigot().sendMessage(Misc.msgHoverClick("§6 ▪ §7/" + getParent().getName() + " " + getSubCommandName() + " remove §o<groupName>",
                "§eRemove an arena group. More details on our wiki.", "/" + getParent().getName() + " " + getSubCommandName() + " remove",
                ClickEvent.Action.SUGGEST_COMMAND));
        p.spigot().sendMessage(Misc.msgHoverClick("§6 ▪ §7/" + getParent().getName() + " " + getSubCommandName() + " §r§7set §o<arenaName> <groupName>",
                "§eSet the arena group. More details on our wiki.", "/" + getParent().getName() + " " + getSubCommandName() + " set",
                ClickEvent.Action.SUGGEST_COMMAND));
    }
